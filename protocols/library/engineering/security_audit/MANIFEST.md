# Application Security Audit Intake — Behavioral Manifest

**Pack ID:** security_audit
...
Governs the intake and scoping of an application security audit — capturing the application scope, threat model, authentication and authorization design, data classification and handling, dependency risk, OWASP Top 10 coverage, penetration testing requirements, and prior vulnerability history to produce a security audit intake profile with risk assessment and recommended audit scope.

Security audits that begin without a threat model are looking for problems without knowing what problems matter. An application that handles financial transactions has a different threat model than an internal HR tool. The OWASP vulnerabilities that are critical for one are irrelevant for the other. The intake establishes the threat model before the audit scope is defined.
...
---
*Application Security Audit Intake v1.0 — TMOS13, LLC*
*Robert C. Ventura*
